...
public class PriorityScheduler extends Scheduler {
	...
	protected class PriorityQueue extends ThreadQueue {
		...
		private void prioritySort() {
			...
			for (int i = 7; i >= 0; i--) { // increment all priorities
				for (KThread thread : waitQueue) { // iterate through thread queue
					if (i == getThreadState(thread).priority) {
						tempQueue.add(thread);
					}
				}
			}
